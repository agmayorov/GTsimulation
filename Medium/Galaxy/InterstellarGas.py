...
class InterstellarGas(GTGeneralMedium):
    ...
    def __calculate_b_field(x, y, z):
        ...
        r_z0 = 8.5
        r_z = 6.94
        w_0p = np.array([2.063e-4, -0.002329, 0.001374, -0.03613])
        w_1p = np.array([-6.838e-5, -0.005807, 0.063540, -0.11340])
        w_2p = np.array([1.217e-4, 0.001202, -0.058400, 0.26010])
        theta_1 = 4.61
        theta_2 = 2.73
        ...
        # --- Warped disk ---
        # Radial profile
        f_d_HI = 0.2291 * r ** 2 * np.exp(-r / 3.016) / 0.250 # normalized on n8
        f_d_CO = np.exp(-(r - r_s) / r_0) * (1 - np.exp(-(r / r_h) ** h_i))

        # Warp of disk
        w_0 = polyval(r, w_0p[::-1])
        w_1 = polyval(r, w_1p[::-1])
        w_2 = polyval(r, w_2p[::-1])
