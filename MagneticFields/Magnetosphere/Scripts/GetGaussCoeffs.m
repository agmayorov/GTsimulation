...
%   Ionospheric magnetic field for CM6, DIFI6
    ...
    if strcmp(GCparam.Model, 'CHAOS') % Chenk number of lines in txt files
        ...
        coefs = getcoefs(Model.TxtFileLoc, 230, 210, 1, 20, 4, 1);
        save(Model.MatFile, '-v7.3', 'coefs');
        
        % Lithospheric field model 21 to 185
        Model = GetBfieldFullModelName('Model', 'CHAOS', 'Ver', GCparam.Ver, 'Type', 'static');
        coefs = getcoefs(Model.TxtFileLoc, 17160, 16995, 21, 185, 4, 1);
        save(Model.MatFile, '-v7.3', 'coefs');
    end
    ...
    if strcmp(GCparam.Model, 'CM')
        ...
        coefs = getcoefs(Model.TxtFile, 189, 171, 1, 18, 4, 1);
...
%         save(Model.MatFile, 'coefs');
    ...
    if strcmp(GCparam.Model, 'COV-OBS')
        ...
        coefs = getcoefs(Model.TxtFile, 119, 105, 1, 14, 7, 1);
        save(Model.MatFile, 'coefs');
    end
    ...
    if strcmp(GCparam.Model, 'LCS')
        ...
        coefs = getcoefs(Model.TxtFile, 17389, 17205, 1, 185, 4, 1);
        save(Model.MatFile, 'coefs');
    end
    ...
    if strcmp(GCparam.Model, 'IGRF')
        ...
        [s, sg, sh, gN, gM, hN, hM, g, h, gh] = initialization(104, 91, length(year));
        for n = 1:13
            for m = 0:n
                sg = sg+1;
                L = strsplit(fgetl(fid));
...
function coefs = getcoefs(filename, n, m, kmin, kmax, r, p)
    fid = fopen(filename);
    ...
    %[s, sg, sh] = deal(0);
    [s, sg, sh, gN, gM, hN, hM, g, h, gh] = initialization(n, m, length(year));
    for n = 1:kmax
        for m = -n:n
            s = s + 1;
            if n < kmin
...
function [s, sg, sh, gN, gM, hN, hM, g, h, gh] = initialization(n, m, y)
    s = 0;
    [sg, sh] = deal(0);
    [gN, gM] = deal(n);
    [hN, hM] = deal(m);
