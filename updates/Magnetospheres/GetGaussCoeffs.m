...
%         save(Model.MatFileLoc, 'coefs');
    ...
    if strcmp(GCparam.Model, 'IGRF')
        ...
        [s, sg, sh, gN, gM, hN, hM, g, h, gh] = initialization(104, 91, 13, 14, length(year));
        for n = 1:13
            for m = 0:n
                sg = sg+1;
                L = strsplit(fgetl(fid));
...
function coefs = getcoefs(filename, n, m, kmin, kmax, r, p)
    ...
    %[s, sg, sh] = deal(0);
    [s, sg, sh, gN, gM, hN, hM, g, h, gh] = initialization(n, m, kmax, kmax+1, length(year));
    for n = 1:kmax
        for m = -n:n
            s = s + 1;
            if n < kmin
...
function [s, sg, sh, gN, gM, hN, hM, g, h, gh] = initialization(n, m, p, q, y)
    s = 0;
    [sg, sh] = deal(0);
    [gN, gM] = deal(n);
    [hN, hM] = deal(m);
