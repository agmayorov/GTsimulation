...
def t15B(parmod, ps, x, y, z):
    ...
    bxr1a_shld, byr1a_shld, bzr1a_shld = r1_a_shld(xx, yy, zz, ps, bzimf, theta00, dtheta00)
    ...
    bypen, bzpen = a[10] * byimf, a[10] * bzimf
...
def tail15_shld(x, y, z, bzimf, pw, xc, rn):
    ...
    a = np.array([
        ...
        -6.4069034, 4.1626399, 0.11085764E-1, 0.24442371E-1,
        -1.4296991, -1.5673126, 0.25464023E-02,-0.10305815E-03,
        0.68969354E-06,-0.33691538E-03, 0.91769431E-03,  6.7307586,
        -2.2051366  , 0.38801141E-05, 0.50020353E-05, 0.26978491E-04,
        -0.87027859E-04, -0.63041822E-02,-0.27773576E-02,-0.13934338E-02,
        -0.16410856
    ])
    ...
    a0 = (a[nlin + 1] + a[nlin + 6] * fbz + a[nlin + 7] * fbz21 + a[nlin + 13] * frn + a[nlin + 14] * fxc +
          a[nlin + 15] * fpw)
    b0 = (a[nlin + 2] + a[nlin + 8] * fbz + a[nlin + 9] * fbz21 + a[nlin + 16] * frn + a[nlin + 17] * fxc +
          a[nlin + 18] * fpw)
...
def brbtbp_prc(r, t, p):
    ...
    d_bit_dt = (
            ...
            + s6 / d6 * abt[35] * f3_48 ** (abt[35] - 1.0) * df3_48_dal * dal_dt * (1.0 - alpha) ** abt[37]
...
def prc_shld_nm(x, y, z, bzimf, eps, scale):
    ...
    a = np.array([
        ...
        0.022048751, 0.061515464, 0.00056954579, -0.00098457765, -0.033742397
    ])
...
def prc_shld_dd(x, y, z, bzimf, eps, scale):
    ...
    a = np.array([
        ...
        0.028940081, 0.048745244, 0.27521861, 0.0000019257827, -0.030864086
    ])
...
@jit(fastmath=True, nopython=True)
def r1_a_shld(x, y, z, psi, bzimf, th, dth):
    nlin = 600
    ntot = 620
    ...
    a = np.array([
        ...
        0.0093470292, 0.000046516521, 0.00051572626, -0.0012140839, 0.000013652830, 0.067482845, -0.0043183269,
        0.0010169675, 0.000014055113, 0.00087104557, 0.026922377, -0.0010044979, 0.0014457557, 0.000060191484,
        0.00020030915, 0.054593113, -0.0014994350, 0.0025290933, 0.0023759393, 0.0017313238
    ])
