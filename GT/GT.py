...
class GTSimulator(ABC):
    ...
    :param Step: The time step of simulation in seconds. If `dict` one should pass:\n
        ...
        3. `MinCyclPeriod`: `int` --- The minimal number of points during one period of cyclotron period\n
        3. `MaxCyclPeriod`: `int` --- The maximal number of points during one period of cyclotron period
    ...
    def __SetStep(self, Step):
        if isinstance(Step, (int, float)):
            ...
            self.N1 = Step.get("MinCyclPeriod", 100)
            self.N2 = Step.get("MaxCyclPeriod", 1000)
        ...
        if self.Verbose:
            if not self.UseAdaptiveStep:
                ...
                print(f"\tMinimal number of steps in cyclotron period: {self.N1}")
                print(f"\tMaximal number of steps in cyclotron period: {self.N2}")
        ...
        for self.index in range(len(self.Particles)):
            ...
            for i in range(Num):
                ...
                if self.UseAdaptiveStep:
                    Step = self.AdaptStep(q, m, np.linalg.norm(B), Step, 100, 1000)
                    q = Step * Q / 2 / m
