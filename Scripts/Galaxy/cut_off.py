...
def find_cut_off(energies, times, times_std):
    nan = np.isnan(times)
    ...
    # plt.errorbar(energies*1e6, times * sec2year, yerr=times_std * sec2year, fmt='-o', capsize=4, label="Modeled data")
    # plt.plot(energies*1e6, func_s(energies, *popt1), label="Fit $t_{min}+\\frac{a}{1+(E/E_0)^\gamma)}$")
    ...
    # plt.vlines(E0*1e6, ymin=np.min(times * sec2year) / 2, ymax=2 * np.max(times * sec2year), colors="black", label="Cut-off energy")
    # plt.xscale("log")
    # plt.yscale("log")
    # plt.xlabel("E, eV")
    # plt.ylabel("t, year")
...
paths = [f"../../tests/Galaxy_newBC{i}" for i in range(1, 9)]
time_energy = dict()
num = 0
...
for path in tqdm.tqdm(paths):
    ...
    for file in files:
        ...
        for event in events:
            ...
            # if phi<0:
            #     phi = 2*np.pi + phi
            if T in time_energy.keys():
                time_energy[T][0].append(time)
                time_energy[T][1].append(theta)
                time_energy[T][2].append(phi)
            else:
                time_energy[T] = [[time], [theta], [phi]]
...
for i in range(N-1):
    for j in range(N-1):
        ...
        phis_std = []
        number = 0
        for energy in sorted(time_energy):
            energies.append(energy)
            time_E = np.array(time_energy[energy][0])
            theta_E = np.array(time_energy[energy][1])
            phi_E = np.array(time_energy[energy][2])
        ...
        # print(len(energies))
        E0s[j, i] = find_cut_off(energies, times, times_std)
