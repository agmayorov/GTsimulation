...
def calc_grid(r_param, theta_param, phi_param, a,
    ...
    for i in prange(Nx):
        for j in prange(Ny):
            for l in prange(Nz):
                r, _, theta, phi = transformations.Cart2Sphere(x[i], y[j], z[l])
                Bx[i, j, l], By[i, j, l], Bz[i, j, l] = calc_noise(r_param, theta_param, phi_param, a,
                                                                   r, theta, phi,
                                                                   A_rad, alpha_rad, delta_rad,
                                                                   A_azimuth, alpha_azimuth, delta_azimuth,
                                                                   A_2d, alpha_2d, delta_2d,
...
if __name__ == "__main__":
    p = ParkerUniform(1, 1, 0, use_reg=False, use_noise=True, use_2d=True, use_slab=False)
    ...
    # plt.colorbar()
    # plt.vlines([2*np.pi/ls], ymin=np.min(Pz), ymax=np.max(Pz))
    # plt.show()
